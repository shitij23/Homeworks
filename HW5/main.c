 ...
 *
 * Created on April 11, 2015, 4:53 PM
 */
...
//sprintf(message,"x: %d y: %d z: %d \r\n a: %d b: %d c: %d \r\n t %d", accels[0],accels[1],accels[2],mags[0],mags[1],mags[2],temp);
     ...
     for(column=0;column<5;column++){
        ...
        for(r=0;r<8;r++){
        x=binary(value,r);
        display_pixel_set(startr+r,startc+column+i*5,x);
        }
...
// Helper functions to get out the binary version of value
int binary(int value1, int position){
    int nmbr;
    int a;
    nmbr =value1>>position;
    if(nmbr&1)
        a=1;
    else a=0;
    return a;
