            ...
            if(appData.emulateMouse)
            {
                ...
                if(movement_length > 50)
                {
                    ...
                    while(i<20){
                        ...
                        x_acc += (int)((double)accels[0]*683/16000);
                        y_acc += (int)((double)accels[1]*384/16000);
                        i++;}
                        x_acc = -x_acc/(20*50);
                        y_acc = -y_acc/(20*50);
           
                        if(x_acc>=683)x_acc=683;
                        if(x_acc<=-683)x_acc=-683;
                        if(y_acc>=384)y_acc=384;
                        if(y_acc<=-384)y_acc=-384;
                        display_clear();
                        oled_pos_set(28,32);
                        sprintf(string,"x:%d y: %d",x_acc,y_acc);
                        print_string_oled(string);
                        display_draw();
                    appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                    appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
                    appData.xCoordinate +=x_acc;//1366 x 768screen//(int8_t)dir_table[vector & 0x07] ;
                    appData.yCoordinate +=y_acc;//(int8_t)dir_table[(vector+2) & 0x07];
                    vector ++;
                    movement_length = 0;
                }
            }
