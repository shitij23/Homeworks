...
short elemsMAF(pointer, accz);
short elemsFIR(pointer, accz);
short elemsMAF[5],elemsFIR[11]; // global variable
unsigned char pointerMAF = 0, pointerFIR=0, flagMAP = 0, flagFIR = 0; // pointer
float FIRcoefficients[11] = { 0.0134, 0.0292, 0.0712, 0.1246, 0.1687, 0.1858, 0.1687, 0.1246, 0.0712, 0.0292, 0.0134 };
...
void APP_Tasks (void )
{
    ...
    unsigned char i,row,szfir,szraw,sz;
    signed char zfir,zraw,z;
    unsigned int j;
    short accels[3]; // accelerations for the 3 axes
    short elems[11],zfilter,zfirfilter;
            ...
            else if( appData.hidDataReceived )
            {
                ...
                switch(appData.receiveDataBuffer[0])
                {
                    case 0x80:
                        ...
                        // the last message is numerical number /0 
                        // the while loop will stop automatically
                        i = 0;
                        while (appData.receiveDataBuffer[i+2])
                        {
                            message[i] = appData.receiveDataBuffer[i+2];
                            i = i + 1;
                        }
                        ...
                        while(j<=1000)
                        {
                            if (_CP0_GET_COUNT()>=800000)
                            {
                                ...
                                sprintf(message,"accx = %4.2f", (accels[0]*1.0)/1600);
                                display_message(message,15,1);
                                sprintf(message,"accy = %4.2f", (accels[1]*1.0)/1600);
                                display_message(message,25,1);
                                sprintf(message,"accz = %4.2f", (accels[2]*1.0)/1600);
                                display_message(message,35,1);
                                display_draw();
                                
								// Utilizing Filter algorithm
								zfilter = elemsMAF(pointerMAF, accels[2]);
								zfirfilter = elemsFIR(pointerFIR, accels[2]);

                                if (zfirfilter>=0){szfir = 1;}
                                else{szfir = 0;}
                                if (accels[2]>=0){szraw = 1;}
                                else{szraw = 0;}
                                if (zfilter>=0){sz = 1;}
                                else{sz = 0;}
                            
                                zfir = (zfirfilter*1.0*(2*szfir-1))/160;
                                zraw = (accels[2]*1.0*(2*szraw-1))/160;
                                z = (zfilter*1.0*(2*sz-1))/160;
                            
                                appData.transmitDataBuffer[0] = szfir;
                                appData.transmitDataBuffer[1] = zfir;
                                appData.transmitDataBuffer[2] = szraw;
                                appData.transmitDataBuffer[3] = zraw;
                                appData.transmitDataBuffer[4] = sz;
                                appData.transmitDataBuffer[5] = z;
...
short elemsMAF(pointerMAF, accz)
{
	short zfilter = 0;
	if (pointerMAF == 0 && flagMAP == 0)
	{
		elemsMAF[pointerMAF] = accz;
		pointerMAF = pointerMAF + 1;
		zfilter = accz;
	}
	else if (pointerMAF >= 1 && pointerMAF <= 5 && flagMAP == 0)
	{
		elemsMAF[pointerMAF] = accz;
		for (i = 0; i <= pointer; i++)
		{
			zfilter = zfilter + elemsMAF[i];
		}
		zfilter = zfilter / i;
		pointerMAF = pointerMAF + 1;
	}
	else if (pointerMAF == 5)
	{
		pointerMAF = 0;
		flagMAP = 1;
		elemsMAF[pointerMAF] = accz;
	...
	else if (pointerMAF != 5 && flagMAP == 1)
	{
		elemsMAF[pointerMAF] = accz;
		for (i = 0; i <= 4; i++)
		{
...
short elemsFIR(pointerFIR, accz)
{
	short zfilter = 0;
	if (pointerFIR == 0 && flagMAP == 0)
	{
		elemsFIR[pointerFIR] = accz;
		pointerFIR = pointerFIR + 1;
		zfilter = accz;
	}
	else if (pointerFIR >= 1 && pointerFIR <= 10 && flagMAP == 0)
	{
		elemsFIR[pointerFIR] = accz;
		for (i = 0; i <= pointer; i++)
		{
			zfilter = zfilter + elemsFIR[i];
		}
		zfilter = zfilter / i;
		pointerFIR = pointerFIR + 1;
	}
	else if (pointerFIR == 10)
	{
		pointerFIR = 0;
		flagMAP = 1;
		elemsFIR[pointerFIR] = accz;
		for (i = 0; i <= 10; i++)
		{
			zfilter = zfilter + elemsFIR[i] * FIRcoefficients[i];
		}
		pointerFIR = pointerFIR + 1;
	}
	else if (pointerFIR != 10 && flagMAP == 1)
	{
		elemsFIR[pointerFIR] = accz;
		for (i = 0; i <= 10; i++)
		{
